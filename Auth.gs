/**
 * Auth.gs — Adaptador de autenticacao do Sano Agent - Quem canta seus males espanta.
 *
 * Conformidade: Prompt 18 - Padronização de Autenticação em Frota
 * Aplicado em: 27/06/2026
 *
 * Arquivos em conformidade:
 * - AuthHelpers.gs (11 rotinas padrão reutilizáveis)
...
function Auth_service_() {
  return AuthStandardService.configure({
    sessionKey: 'SANO_AGENT___QUEM_CANTA_SEUS_MALES_ESPANTA_AUTH_SESSION',
...
var AUTH_LEGACY_TOK_TTL_MS_ = 21600 * 1000; // 6 horas

/** Obtem ou cria a aba SessoesAuth (token,userId,username,role,expiresAt). */
function getLegacySessoesAuthSheet_() {
  try {
    var ss = Auth_getSpreadsheet_();
    if (!ss) return null;
    ...
    Logger.log("Erro em getLegacySessoesAuthSheet_: " + error.message);
 ...
 */
function loginWithTokenLegacy_(username, password) {
  try {
    try {
      if (!String(username || '').trim() || !String(password || '')) {
        return { success: false, message: 'Informe usuario e senha.' };
      ...
      var sheet = getLegacySessoesAuthSheet_();
      if (!sheet) return { success: false, message: 'Erro ao criar sessao.' };

      var token = Utilities.getUuid().replace(/-/g, '');
      var expiresAt = new Date().getTime() + AUTH_LEGACY_TOK_TTL_MS_;
      sheet.appendRow([
        token,
 ...
 */
function isAuthenticatedByTokenLegacy_(tok) {
  try {
    try {
      try {
        ...
        for (var i = 1; i < data.length; i++) {
          if (String(data[i][0]).trim() === normalizedToken) {
            ...
            if (!isFinite(expiresAt) || expiresAt <= now || !String(data[i][1] == null ? '' : data[i][1]).trim()) {
              sheet.deleteRow(i + 1);
              return false;
            }
            return true;
 ...
 */
function getSessionUserLegacy_(tok) {
  try {
    try {
      try {
        var normalizedToken = String(tok == null ? '' : tok).trim();
...
function logoutWithTokenLegacy_(tok) {
  try {
    try {
      try {
        var normalizedToken = String(tok == null ? '' : tok).trim();

 ...
 *
 * FROTA-PROMPT-06: Token propagado via payload._authToken; validado antes de
 * operacoes autenticadas; extraido e resolvido por gw_resolveAuthenticatedPrincipal_.
 *
...
function apiCall(service, method, payload) {
  try {
    ...
    try {
      if (!publicCall && !gw_resolveAuthenticatedPrincipal_(request)) {
      ...
      switch (operation) {
        ...
        case 'AuthService.logout':
          ...
          if (data && data.success === false) throw new Error(data.error || data.message || 'Não foi possível encerrar a sessão.');
          break;
        default:
...
function gw_login_(username, password) {
  try {
    ...
    if (typeof handleLogin === 'function')              entries.push(function (form) { return handleLogin.apply(null, form); });
...
/** Resolve o logout pelo token recebido; wrappers legados ficam apenas como fallback sem concessao. */
function gw_logout_(payloadOrToken) {
  var token = '';
  if (typeof payloadOrToken === 'string') {
    token = payloadOrToken;
  } else if (payloadOrToken && typeof payloadOrToken === 'object') {
    token = payloadOrToken._authToken || payloadOrToken.tok || '';
  }
  try {
    if (typeof logoutWithToken === 'function') return logoutWithToken(token);
    if (typeof doLogout === 'function') return doLogout(token);
    if (typeof logout === 'function') return logout(token);
  } catch (ignored) {}
  return { success: false, error: 'Serviço de logout não disponível.' };
}

/**
 * FROTA-PROMPT-06: Resolve o principal autenticado a partir do payload.
 * 
 * Prioridade:
 * 1. Token no payload (payload._authToken ou payload.tok)
 * 2. Sem token valido, negar acesso. Sessoes legadas nao autorizam o gateway.
 * 
 * @param {Object} payloadOrToken - Payload com _authToken ou string de token
 * @returns {Object|null} Principal autenticado ou null
 */
function gw_resolveAuthenticatedPrincipal_(payloadOrToken) {
  // 1. Extrai token do payload
  var token = null;
  if (typeof payloadOrToken === 'string') {
    token = payloadOrToken;
  } else if (payloadOrToken && typeof payloadOrToken === 'object') {
  ...
  // 2. Valida token via getSessionUser (presente em Auth.gs)
  if (token) {
    try {
  ...
  // Sem token valido, falha fechada. Wrappers legados podem existir, mas nao concedem acesso aqui.
  return null;
}
 ...
 * 
 * NOTA: Esta funcao e um FALLBACK. O padrao e usar gw_resolveAuthenticatedPrincipal_
 * que valida o token do payload.
 */
...
function gw_normalizeLogin_(r) {
  if (!r || typeof r !== 'object') return { success: false, error: 'Resposta de autenticação inválida.' };
  ...
  return { success: false, error: 'Resposta de autenticação sem sessão.' };
}

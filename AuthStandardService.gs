...
var AuthStandardService = (function () {
  ...
  function configure(options) {
    ...
    if (options.sessionTtlSeconds !== undefined) {
      var ttl = Number(options.sessionTtlSeconds);
      if (isFinite(ttl) && ttl > 0) config_.sessionTtlSeconds = ttl;
    }
    return api;
  }
  ...
  function getSessionByToken_(token) {
    try {
      ...
      try {
        ...
        if (!session || !String(session.userId == null ? '' : session.userId).trim() || !isFinite(expiresAt) || expiresAt <= now_()) {

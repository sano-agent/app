...
function getInitialAppData(tok) {
  ...
  var bootstrap = (typeof sanoClientApi === 'function')
    ? sanoClientApi('bootstrap', tok, {})
    : { success: false, error: 'Serviço de dados iniciais não disponível.' };
  if (!bootstrap || bootstrap.success === false) {
    return { success: false, message: (bootstrap && (bootstrap.error || bootstrap.message)) || 'Não foi possível carregar os dados iniciais.' };
  }
  var data = bootstrap.data || {};
  return { success: true, currentUser: data.user || currentUser, appData: data.stats || {} };
}
